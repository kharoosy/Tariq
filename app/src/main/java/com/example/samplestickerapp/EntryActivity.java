...
public class EntryActivity extends BaseActivity {
    ...
    private void showErrorMessage(String errorMessage) {
        progressBar.setVisibility(View.GONE);
        if ("NO_PACKS_YET".equals(errorMessage)) {
            // Fresh install, nothing imported yet - go straight to the import screen
            // instead of showing an error.
            startActivity(new Intent(this, ImportStickersActivity.class));
            finish();
            overridePendingTransition(0, 0);
            return;
        }
            ...
            try {
                ...
                if (context != null) {
                    ...
                    if (stickerPackList.size() == 0) {
                        return new Pair<>("NO_PACKS_YET", null);
                    }
                ...
                if (e.getMessage() != null && e.getMessage().contains("at least one sticker pack")) {
                    // Fresh install / nothing imported yet - not a real error.
                    return new Pair<>("NO_PACKS_YET", null);
                }
